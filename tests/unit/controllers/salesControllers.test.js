...
describe('Desenvolve os testes para a camada de sales da Controllers', () => {
  ...
  describe('Verifica a função findAllSales', () => {
    ...
    it('Testa a função findAllSales em caso de sucesso', async () => {
      ...
      res.status = sinon.stub().returns(res);
      res.json = sinon.stub().returns(payloadSales);
  ...
  describe('Verifica a função findSaleById', () => {
    it('Testa a função findSaleById em caso de falha', async () => {
      const payload = {
        status: 404,
        message: 'Sales not found',
      };
      ...
      const payloadId = 1;
      sinon.stub(salesServices, 'findSaleById').returns(payload);

      const response = await salesServices.findSaleById(payloadId);
      await salesControllers.findSaleById(req, res);
      expect(res.json.calledWith({ message: response.message })).to.be.equal(
        true
      );
      expect(res.status.calledWith(404)).to.be.equal(true);
    ...
    it('Testa a função findSaleById em caso de sucesso', async () => {
      const payloadSales = [
        { date: '2022-07-06T12:46:08.000Z', productId: 1, quantity: 5 },
      ];
      res.status = sinon.stub().returns(res);
      res.json = sinon.stub().returns(payloadSales);
      req.params = sinon.stub().returns({ id: 1 });
      const payloadId = 1;
      sinon.stub(salesServices, 'findSaleById').returns(payloadSales);
